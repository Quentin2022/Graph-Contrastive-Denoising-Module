...
def contrastive_loss(embeddings, labels, num_nodes, metric, loss_type, margin, gamma=0.001):
...
def train_pretraining_model(train_loader, test_loader, num_nodes, epochs=30, hidden_layers=[64, 64], 
                            dropout_rate=0.5, sim_metric='CosSimilarity', margin=0.3, pretraining_lr=0.001, 
                            pretraining_weight_decay=0, embeddings_loss_type='MarginBased', if_plotadj=False,
                            train_graphs=None, test_graphs=None):
    ...
    for epoch in range(epochs):
        ...
        for _, labels, graphs in train_loader:
            ...
            loss = contrastive_loss(embeddings, labels, num_nodes, metric=sim_metric, margin=margin, loss_type=embeddings_loss_type)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    model.eval()
    train_graphs_flattened = train_graphs[:, idx[0], idx[1]].to(device)
    test_graphs_flattened = test_graphs[:, idx[0], idx[1]].to(device)
    embeddings_train = model(train_graphs_flattened)
    embeddings_test = model(test_graphs_flattened)
    trans_train_graphs = torch.zeros_like(train_graphs).to(device)
    trans_test_graphs = torch.zeros_like(test_graphs).to(device)
    trans_train_graphs[:, idx[0], idx[1]] = embeddings_train
    trans_train_graphs[:, idx[1], idx[0]] = embeddings_train
    trans_test_graphs[:, idx[0], idx[1]] = embeddings_test
    trans_test_graphs[:, idx[1], idx[0]] = embeddings_test

    return train_losses, test_losses, model_params_list, trans_train_graphs.detach().cpu(), trans_test_graphs.detach().cpu()
