...
class GraphIsomorphism(nn.Module):
    def __init__(self, input_dim, output_dim, num_nodes, ifTransAdj, TransAdj, 
        ...
        # Initialize the MLP of GIN
        hidden_dim = (input_dim + output_dim) * 3 // 2
    ...
    def forward(self, input, adj):
        if self.graph_embedding:
            ...
            adj = adj + torch.eye(self.num_nodes, device=input.device)

        aggregate = torch.matmul(adj, input)
        output = self.mlp(aggregate)  # The MLP used here is not the same as the one in GraphEmbeddings
        return output
...
class GIN(nn.Module):
    def __init__(self, layer_sizes, dropout=0.5, readout='fc', num_classes=2, num_nodes=16, ifTransAdj=True, TransAdj='A',
        ...
        for i in range(len(layer_sizes) - 1):
            self.layers.append(GraphIsomorphism(layer_sizes[i], layer_sizes[i + 1], num_nodes, ifTransAdj, TransAdj,
                                                embedding_hidden_layers, embedding_initial_weights, embedding_if_finetune))
